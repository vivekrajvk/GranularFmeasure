...
%SOLVE_KNAPSACK Summary of this function goes here
    ...
    if portion>1%for Cosum dataset evaluation
        amount=selectShot(shot_scores,portion);
        portion=portion/100;
    else
...
function amt=selectShot(shot_scores,noShots)
amt = zeros(length(shot_scores),1);
[~,ind] = sort(shot_scores,'descend');
amt(ind(1:min(noShots,length(shot_scores))))=1;%select top 'noShots' as skim
